...
class IKFKSolver:
    ...
    def compute_abs_eef_in_base(self, arm_joint_states, use_gripper_offset=True):
        """
        Compute absolute end-effector pose in base frame
        
        Args:
            arm_joint_states: Joint positions for both arms (14 values)
            use_gripper_offset: If True, compute gripper center pose (arm end + 143mm offset)
                              If False, compute arm end pose only
        
        Returns:
            left_xyzrpy, right_xyzrpy: xyz + rpy for left and right arms
        """
        ...
        if use_gripper_offset:
            """
            gripper -> base = base -> center @ arm end -> center @ gripper center -> arm end
            """
            ...
            """
            arm end -> base = base -> center @ arm end -> center
            """
    ...
    def compute_abs_eef_in_base_quat(self, arm_joint_states, use_gripper_offset=True):
        """
        Compute absolute end-effector pose in base frame with quaternion
        ...
        Returns:
            left_xyzquat, right_xyzquat: xyz + quaternion (scalar first) for left and right arms
        """
        left_xyzrpy, right_xyzrpy = self.compute_abs_eef_in_base(arm_joint_states, use_gripper_offset)
        left_xyzquat = xyzrpy_to_xyzquat(left_xyzrpy)
        right_xyzquat = xyzrpy_to_xyzquat(right_xyzrpy)
        return left_xyzquat, right_xyzquat
